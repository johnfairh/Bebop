...
internal enum L10n {
  internal enum Localizable : String {
    ...
    /// Both --bebop-json-files and other data-source options are set: choose just one.
    case errCfgBebopJsonMutex = "err-cfg-bebop-json-mutex"
    ...
    /// SourceKitten couldn't find build info from `xcodebuild`.  Check the log file mentioned above: it was looking for a `swiftc` command that included `-module-name`. You might need to experiment running `xcodebuild` manually to find the set of flags (perhaps a `-scheme` setting?) required to build your module, and then pass through those flags to this program using --build-tool-arguments.
    case errSktnXcodeDef = "err-sktn-xcode-def"
    /// SourceKitten couldn't find build info from `xcodebuild`.  Check the log file mentioned above; it was looking for a `swiftc` command that included `-module-name %1`. You might need to experiment running `xcodebuild` manually to find the set of flags (perhaps a `-scheme` setting?) required to build your module, and then pass through those flags to this program using --build-tool-arguments.
    ...
    /// bebop: Generate API documentation for Swift or Objective-C modules.\n\nUsage: bebop [options]\n\nOptions:
    ...
    case wrnBebopJsonDecode = "wrn-bebop-json-decode"
    /// Can't import Bebop JSON file '%1', version '%2' is from a later version of the program.
    case wrnBebopJsonFuture = "wrn-bebop-json-future"
