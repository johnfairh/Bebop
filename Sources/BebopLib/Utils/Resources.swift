...
public final class Resources {
    ...
    public static let BUNDLE_ENV_VAR = "BEBOP_RESOURCES_PATH"
    ...
    /// BebopLib framework bundle.  This is (sort of) how the APIs want this to work.
    static func xcodeFindMainBundle() -> Bundle {
        ...
        guard let pluginsURL = frameworkBundle.builtInPlugInsURL,
            case let resourceBundleURL = pluginsURL.appendingPathComponent("BebopResources.bundle"),
            let resourceBundle = Bundle(url: resourceBundleURL) else {
            preconditionFailure("Packaging looks broken, can't find 'Plugins/BebopResources.bundle' " +
    ...
    static func spmFindMainBundle() -> Bundle {
        ...
        let bundleFileName = "bebop.resources"
        ...
        guard binaryDirectory.lastPathComponent == "bin" else {
            preconditionFailure("Installation looks broken, not installed in a 'bin' directory " +
                "or with 'bebop.resources' in the same directory as the executable, " +
        ...
        guard let resourceBundle = Bundle(url: fhsBundle) else {
            preconditionFailure("Installation looks broken, executable in 'bin' directory " +
                "\(binaryDirectory.path) but can't find 'bebop.resources' bundle at \(fhsBundle.path)")
        }
