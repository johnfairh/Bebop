...
class TestConfig: XCTestCase {
    ...
    func testConfigFile() throws {
        ...
        let configFile = try tmpDir.createFile(name: ".bebop.yaml")
    ...
    // 2b Config file - Check bebop->jazzy and parent-searching
    func testConfigFileSearch() throws {
        let tmpDir = try TemporaryDirectory()
        let tmpSubDir = try tmpDir.createDirectory()
        let bebopConfig = try tmpDir.createFile(name: ".bebop.yaml")
        try "name: barney".write(to: bebopConfig)
    ...
    func testSrcDirConfigFile() throws {
        let tmpDir = try TemporaryDirectory()
        let configFile = tmpDir.directoryURL.appendingPathComponent(".bebop.yaml")
