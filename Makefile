...
test_linux:
	docker run -v `pwd`:`pwd` -w `pwd` --name bebop --rm swift:5.9 /bin/bash -c "apt-get update; apt-get install make libsqlite3-dev libsass1 libsass-dev; make test"

shell_linux:
	docker run -it -v `pwd`:`pwd` -w `pwd` --name bebop --rm swift:5.9 /bin/bash
